...
public class TweetReader {
	...
	private static final String screenName = "remaster";
	...
	private static ArrayList<JSONObject> convertTimelineToJson(List<Status> statuses) {
		...
		try {
			if (statuses.size()>0){
				...
				JSONObject object = new JSONObject();
				object.put("tweet", "No te has logueado aun por favor haz login para ver tus tweets!");
				object.put("author", "");
				JOBS.add(object);
			}

		} catch (JSONException e1) {
			Log.e("JSON", "A ocurrido un error creando el JSONObject", e1);
		}
		return JOBS;
	}
}
